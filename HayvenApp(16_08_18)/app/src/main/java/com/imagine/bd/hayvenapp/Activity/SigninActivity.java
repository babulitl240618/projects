            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                try {
                    ...
                    if (object.getMsg().equalsIgnoreCase("true")) {
                        ...
                        // Log.e("size1", object.getChatuserlist().size() + " tst");
...
/*
                        ...
                        PersistData.setStringData(con, AppConstant.userImg, object.getUser().getImg());
                        PersistData.setStringData(con, AppConstant.userPhoto, object.getUser().getImg());
                        ...
                        if (object.getUnreadConv().size() > 0){

                            int count = 0;
                            for (int i = 0; object.getUnreadConv().size() > i; i++) {
                                count = count + Integer.parseInt(object.getUnreadConv().get(i).getTotalUnread());
                            }

                            Log.e("badge", String.valueOf(count));
                            int badgeCount = count;

                            if(count>0){
                                ShortcutBadger.applyCount(con, badgeCount); //for 1.1.4+
                            }else{
                        ...
                        *//* all userlist save in sharepreference*//*
                        ...
                        *//* all last conversation save in sharepreference*//*

                        if (object.getUnreadConv().size() > 0){

                            AppConstant.myUserList = object.getUnreadConv();
                            int count = 0;
                            for (int i = 0; object.getUnreadConv().size() > i; i++) {
                                count = count + Integer.parseInt(object.getUnreadConv().get(i).getTotalUnread());
                            }
